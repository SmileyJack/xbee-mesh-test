...
def data_rcvd_callback(xbee_message):
    ...
    calculate_and_record_message_and_response_time(data)

    
def calculate_and_record_message_and_response_time(data):
    global response_log
    global receive_log
    global send_log
    ...
    if (node_id not in receive_log.keys()):
        response_log[node_id] = Agent(node_id, [new_tuple])
    else:
        response_log.get(node_id).responses.append(new_tuple)
